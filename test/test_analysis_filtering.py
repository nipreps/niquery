...
def test_filter_runs():
    ...
    # Timepoint filtering leaves 200,300,150 then contribution limits ds1 to 2 rows total
    assert len(out) == 3
...
def test_identify_relevant_runs():
    ...
    # Same expectations as filter_runs; ordering may differ due to shuffle
    assert len(out) == 3
    assert set(out[VOLS]) == {150, 200, 300}
